...
class Template(object):
    ...
    def make_template(self):
        def check_stopword(w):
            if w.name not in stopwords.words('english'):
                w.name = None
            return w
        template = [[check_stopword(w) for w in l]
                    for l in self._token_template]
        return template
    ...
    def make_syllables_template(self):
        syllables = [sum([s.syllables for s in t])
                     for t in self._token_template]
        return syllables
