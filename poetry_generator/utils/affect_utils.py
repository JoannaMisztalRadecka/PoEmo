...
import poetry_generator.settings as settings
import utils
...
# Alec Larsen - University of the Witwatersrand, South Africa, 2012 import
# shlex, subprocess
...
def calculate_valence(text):
    if text == '':
        return []
    # open a subprocess using shlex to get the command line string into the
    # correct args list format
    p = subprocess.Popen(
        shlex.split(
            "java -jar {0}/SentiStrength.jar stdin exclamations2 noIgnoreBoosterWordsAfterNegatives sentidata {0}/SentStrength/". format(
                settings.resources_dir)),
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE)
    # communicate via stdin the string to be rated. Note that all spaces are
    # replaced with +
    stdout_text, stderr_text = p.communicate(text.replace(" ", "+"))
    # remove the tab spacing between the positive and negative ratings. e.g. 1
    # -5 -> 1-5
...
def make_wn_affect_tree():
    affect_hierarchy_xml = minidom.parse(
        settings.WN_affect_hierarchy_filename)
...
def read_wn_affect_noun_synsets():
    affect_synsets_xml = minidom.parse(settings.WN_affect_synsets_filename)
...
def make_wn_affect_dictionary():
    ...
    noun_dict = {c.attributes['id'].value: c.attributes[
        'categ'].value for c in wn_affect_nouns}
    other_pos_dict = {c.attributes['id'].value: noun_dict[
        c.attributes['noun-id'].value] for c in wn_affect_other_pos}
...
def find_affect_synsets_for_emotion(emotion_name, wn_affect_tree):
    with open(settings.wn_id_dict_filename, 'rb') as handle:
    ...
    affect_synsets = [wn_id_dict[int(syn_id[2:])] for syn_id in wn_affect_syns if wn_affect_syns[
        syn_id] in affect_categories]
...
def calculate_arousal(text):
    ...
    words_arousal = [
        utils.anew_list[
            string.lower(w)][1] -
        4 for w in words if w in utils.anew_list]  # get anew arousal for words
...
def get_emotion(valence, arousal):
    def emotion_distance2d(xxx_todo_changeme, xxx_todo_changeme1):
        (v1, a1) = xxx_todo_changeme
        (v2, a2) = xxx_todo_changeme1
        return math.sqrt((v2 - v1)**2 + (a2 - a1)**2)

    d = {
        em: emotion_distance2d(
            emotions_dict[em],
            (valence,
             arousal)) for em in emotions_dict}
    emotion = min(d, key=d.get)
    return emotion


def get_emotion_distance(valence, arousal, emotion):
    def emotion_distance2d(val_ar1, val_ar2):
        (v1, a1) = val_ar1
        (v2, a2) = val_ar2
        return math.sqrt((v2 - v1)**2 + (a2 - a1)**2)
    try:
