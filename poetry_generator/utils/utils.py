from nltk.corpus import cmudict, WordNetCorpusReader, words, stopwords, RegexpTokenizer
import nltk
import pickle
...
def filter_words(text_file):
    ...
    filtered_words = [w for w in pos_list if not w[
        0] in stopwords.words('english')]
    return filtered_words


def filter_stopwords_tokens(tokens):
    filtered_words = [w for p in tokens for w in p if not w[
        0] in stopwords.words('english')]
...
def get_pos(tokens):
    pos = [[t.pos for t in l]for l in tokens]
    return pos
...
def extend_emotion_lookup():
    ...
    try:
        ...
        try:
            ...
            new_lines = sorted(['\t'.join(l)for l in lines_split_emot])
