...
def SyllableCount(AszWord):
    ...
    return [len([y for y in x if isdigit(y[-1])])
...
def count_word_syllables(word):
  ...
  #  LzzSyllableCounter.Load()
   ...
   # LliList = ['','theatre','productized','productised','pumblechook','everything','altogether','particular','opportunity','everybody','cooeed','cueing']
    ...
    # print "'%s' has max(%d), min(%d), ave(%3.2f), raw(%s) syllables -
    # Calculated by (%s)" %
    # (LszWord,LzzSyllableCounter.SyllableCount(LszWord),
    # LzzSyllableCounter.SyllableCount(LszWord,
    # AszMode='min'),LzzSyllableCounter.SyllableCount(LszWord,
    # AszMode='ave'),LzzSyllableCounter.SyllableCount(LszWord,
